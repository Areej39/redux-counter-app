...
function App() {
  ...
  return (
    <div className='min-h-screen bg-linear-to-br from-cyan-100 via-sky-50 to-blue-100 flex items-center justify-center p-6'>
      <div className='relative w-full max-w-md bg-white rounded-3xl shadow-2xl p-6 sm:p-8 transition duration-300 hover:scale-[1.02] hover:shadow-cyan-300/50'>
        <button onClick={handleRefresh} className='absolute top-4 right-4 p-2 hover:bg-gray-100 rounded-full transition-colors' aria-label="Refresh page">
          <ArrowPathIcon className='w-6 h-6 text-gray-700' />
        </button>

        <h1 className='text-3xl sm:text-4xl font-bold text-gray-800 text-center'>Counter App</h1>
        <p className="text-gray-500 text-center mt-2">
          Manage your counter using Redux Toolkit
        </p>

        <div className='mx-auto my-8 w-36 h-36 sm:w-44 sm:h-44 rounded-full bg-linear-to-r from-cyan-500 to-blue-500 flex items-center justify-center shadow-lg'>
          <h2 className='text-white text-5xl sm:text-6xl font-bold'>{count}</h2>
        </div>

        <div className='flex justify-center mt-6'>
          <Stack spacing={2} direction="row" useFlexGap flexWrap="wrap" justifyContent="center">
